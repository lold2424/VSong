    ...
    public boolean isSongByAI(String title, String description) {
        ...
        try {
            String prompt = String.format(
                "다음 유튜브 영상이 실제 '가수가 노래를 부르는 음악 콘텐츠(커버곡, 오리지널 곡, 공식 MV)'인지 판별해줘.\n\n" +
                "**반드시 'false'로 응답해야 하는 경우 (매우 중요)**:\n" +
                "- 제목이나 설명이 질문형인 경우 (예: ~하는 법?, ~등급?, ~인가요?)\n" +
                "- 단순 게임 방송 하이라이트, 뽑기, 랭킹, 월드컵, 리액션 영상\n" +
                "- 잡담, 일상 브이로그, 정보 제공, 결혼/의학/법률 등 전문 정보\n" +
                "- 1분 내외의 짧은 클립 또는 쇼츠(Shorts)\n" +
                "- 배경음악(BGM)으로 사용된 노래 정보만 적혀 있는 일반 방송\n\n" +
                "**'true'로 응답하는 경우**:\n" +
                "- 가창자가 직접 노래를 부르는 영상\n" +
                "- 공식 뮤직비디오(Official MV)\n\n" +
                "결과는 반드시 'true' 또는 'false' 한 단어로만 응답해.\n" +
                "영상 제목: %s\n" +
